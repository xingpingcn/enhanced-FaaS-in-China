...
> [!IMPORTANT]
>
> `verlify-cname.xingpingcn.top` 已经弃用。见 [#35](https://github.com/xingpingcn/enhanced-FaaS-in-China/issues/35)
...
- 这大概率是使用了`verlify-cname.xingpingcn.top`（已经弃用）导致的。需要先把 CNAME 记录改为官方提供的链接，等待 SSL 证书生成后再重新设置。这是由于该解析包含两个平台的 IP，平台每次访问都会获得二者之一的 IP，因而认为你在平台所填写的域名并不是你所拥有的。但是一旦生成证书后，证书就会缓存在平台上。
...
A：这是因为该路线的其他 IP 质量较差，所以暂时停止解析其路线，改用官方提供的默认 IP。
